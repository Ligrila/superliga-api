...
class QuestionsController extends AdminAppController
{
    ...
    public function finish($question_id,$correct_option){
        $question = null;
        $saved = $this->Questions->setCorrectOption($question_id,$correct_option,$question);
        if(!empty($question->publicity_campaign)){
            if(!empty($question->publicity_campaign)&&!empty($question->publicity_campaign->banner)){
                ...
                $this->Questions->Trivias->showBanner(compact('trivia_id','bannerUri','bannerType','data'));
            }
        }
        if($saved){
