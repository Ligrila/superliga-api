<nav class="nav nav-pills m-b-1">
    <?= $this->Html->link(__('Listado').'<i class="material-icons">list</i>', ['action' => 'index'],['class'=>'btn btn-primary btn-icon','escape'=>false]) ?>
        <?= $this->Form->postLink(
            ...
            ['action' => 'delete', $superuser->id],
            ['class'=>'btn btn-danger btn-icon','escape'=>false,'confirm' => __('¿Estás seguro de borrar el item # {0}?', $superuser->id)]
        )
    ?>
    </nav>
        ...
        <div class="form-body">
          <fieldset>
              <?php
                        ...
                        echo $this->Form->control('last_login', ['empty' => true,'type'=>'text','data-provide'=>'datepicker','data-date-language'=>'es']);
