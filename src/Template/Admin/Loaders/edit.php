<nav class="nav nav-pills m-b-1">
    <?= $this->Html->link(__('Listado').'<i class="material-icons">list</i>', ['action' => 'index'],['class'=>'btn btn-primary btn-icon','escape'=>false]) ?>
        <?= $this->Form->postLink(
            ...
            ['action' => 'delete', $loader->id],
            ['class'=>'btn btn-danger btn-icon','escape'=>false,'confirm' => __('¿Estás seguro de borrar el item # {0}?', $loader->id)]
        )
    ?>
    </nav>
...
<div class="card-block">
      <div class="loaders form large-9 medium-8 columns content">
        <?= $this->Form->create($loader) ?>
        <div class="form-body">
          <fieldset>
              <?php
